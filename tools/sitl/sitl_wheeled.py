...
class RobotSim:
    def __init__(self, world: World, start_x: float = 1000, start_y: float = 1000,
                 start_hdg_deg: float = 0):
        self.world   = world
    ...
    def status_packet(self) -> StatusPacket:
        return StatusPacket(
            mode=1, tasks_ok=8, canary_ok=8,
            uptime_s=int(time.time()),
            robot_type=ROBOT_WHEELED,
        )

    def __repr__(self) -> str:
        return (f"RobotSim(x={self.x:.0f} y={self.y:.0f} "
...
class SITLClient:
    def __init__(self, robot: RobotSim, host: str, port: int,
                 sensor_hz: float = 20, cam_hz: float = 2,
                 duration: float = 0,
    ...
    async def _camera_loop(self, writer: asyncio.StreamWriter):
        ...
        while self._running:
            payload = render_camera_payload(
                ...
                self.robot.hdg_deg, self.robot.world,
            )
            ...
            if frame_count == 1:
                ...
                print(f"[SITL] Camera active: {self.cam_hz} Hz, "
                      f"JPEG ~{jpeg_size} bytes/frame")
            await asyncio.sleep(interval)
...
def main():
    ...
    ap.add_argument("--state-file", default="/tmp/sitl_state.json",
    ...
    client = SITLClient(robot, args.host, args.port,
                        sensor_hz=args.hz, cam_hz=args.cam_hz,
