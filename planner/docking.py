...
class DockManager:
    ...
    def is_docked(self) -> bool:
        return self.state in (
            DockState.DOCKED, DockState.CHARGING, DockState.CHARGED,
        )

    @property
    def needs_charge(self) -> bool:
        return self.state in (
            DockState.LOW_BATTERY, DockState.NAVIGATING,
    ...
    def update_battery(self, battery_mv: int) -> DockState | None:
        ...
        if self.state != prev_state:
            logger.info(
                ...
                prev_state.value, self.state.value, battery_mv,
            )
            return self.state
        return None
    ...
    def report_docked(self):
        ...
        if self.state in (DockState.HOMING, DockState.NAVIGATING,
                          DockState.LOW_BATTERY, DockState.CRITICAL):
